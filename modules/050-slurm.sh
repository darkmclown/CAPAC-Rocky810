...
slurm_main() {
  log_section "Slurm + Munge + MariaDB + Lmod + Intel MPI Module"
  ...
  slurm_write_configs
  slurm_configure_lmod
  slurm_configure_intel_mpi
  ...
  log_ok "Slurm + Munge + MariaDB + Lmod module completed successfully."
}
...
slurm_validate_hostname() {
  ...
  log_info "Current hostname -s: $(hostname -s)"
  log_info "Current hostname -f: $(hostname -f 2>/dev/null || hostname)"

  if [[ "${NODE_ROLE}" == "master" && "$(hostname -s)" != "${MASTER_HOST}" ]]; then
    log_warn "This node is master role but hostname is not ${MASTER_HOST}."
  fi

  if [[ "${NODE_ROLE}" == "compute" && "$(hostname -s)" != "${COMPUTE_HOST}" ]]; then
    log_warn "This node is compute role but hostname is not ${COMPUTE_HOST}."
  fi
}
...
slurm_install_packages() {
  log_section "Installing Slurm, Munge, MariaDB, Lmod and MPI Packages"
  ...
  log_ok "Slurm/Munge/MariaDB/Lmod package installation completed."
}
...
# Generated by CAPAC Rocky 8.10 bootstrap
# ==============================================================================
...
slurm_configure_lmod() {
  log_section "Configuring Lmod and Modulefiles"

  mkdir -p "${MODULEFILES_PATH}"
  chmod 755 "${MODULEFILES_PATH}"

  cat > /etc/profile.d/capac-lmod.sh <<EOF
# CAPAC Lmod module path
...
EOF
  ...
  cat > "${MODULEFILES_PATH}/openmpi/rocky8.lua" <<'EOF'
help([[
...
whatis("Name: OpenMPI")
whatis("Category: MPI")
whatis("Description: OpenMPI from Rocky/EPEL packages")
...
EOF
  ...
  log_ok "Lmod configured with module path: ${MODULEFILES_PATH}"
}
...
slurm_configure_intel_mpi() {
  log_section "Configuring Intel MPI / oneAPI"
...
EOF
    ...
    dnf install -y intel-oneapi-mpi intel-oneapi-mpi-devel || {
      log_warn "Intel MPI package install failed. Modulefile will be created only if path exists."
    }
  fi

  mkdir -p "${MODULEFILES_PATH}/intel-mpi"

  cat > "${MODULEFILES_PATH}/intel-mpi/oneapi.lua" <<EOF
help([[
Intel MPI module for CAPAC CAE/HPC cluster.
Loads Intel oneAPI MPI if installed under ${INTEL_ONEAPI_ROOT}.
]])

whatis("Name: Intel MPI")
whatis("Category: MPI")
whatis("Description: Intel MPI from oneAPI")

local oneapi_root = "${INTEL_ONEAPI_ROOT}"
local mpi_root = pathJoin(oneapi_root, "mpi/latest")

if not isDir(mpi_root) then
  LmodError("Intel MPI not found at " .. mpi_root .. ". Install Intel oneAPI MPI first.")
end
...
-- Good default for Ethernet-based cluster. Adjust later for OFI/IB.
setenv("I_MPI_FABRICS", "shm:tcp")
setenv("I_MPI_PIN", "1")
setenv("I_MPI_PIN_DOMAIN", "core")
EOF

  log_ok "Intel MPI modulefile created: ${MODULEFILES_PATH}/intel-mpi/oneapi.lua"
}
...
slurm_setup_root_ssh_if_master() {
  ...
  if ssh -o BatchMode=yes -o ConnectTimeout=5 root@"${COMPUTE_HOST}" "hostname -s" >/dev/null 2>&1; then
  ...
  if slurm_confirm "Setup root SSH key copy to ${COMPUTE_HOST} now? [y/N]: "; then
    ssh-copy-id root@"${COMPUTE_HOST}" || {
      log_warn "root ssh-copy-id failed."
      log_warn "You may need to enable root SSH or run manually:"
...
slurm_sync_to_compute_if_master() {
  ...
  ssh root@"${COMPUTE_HOST}" "mkdir -p /etc/munge /etc/slurm /var/log/slurm /var/spool/slurmd /home/data/scratch"
  ...
  rsync -av /etc/profile.d/capac-lmod.sh root@"${COMPUTE_HOST}":/etc/profile.d/capac-lmod.sh
...
#SBATCH --output=/home/data/scratch/capac-hostname-%j.out
...
#SBATCH --time=00:05:00
#SBATCH --output=/home/data/scratch/capac-cpu-test-%j.out
...
EOF
  ...
  log_info "After both nodes are configured, run:"
...
slurm_summary() {
  ...
  if [[ "${NODE_ROLE}" == "master" ]]; then
    ...
    log_info "If accounting cluster is missing, run:"
    log_info "  sudo sacctmgr -i add cluster ${CLUSTER_NAME}"
  fi
